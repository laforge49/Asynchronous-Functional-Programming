...
import core.rel.SubjectUuidsSequence

class SubjectUuidsSequenceAgent(systemContext:SystemComposite, uuid:String)
        extends AsynchronousActor(systemContext,uuid) with SequenceWrapper{
  override def wrappedSequence = {
    val value = payload.readUTF
    ...
    else SubjectUuidsSequence(rolon, relType, value)
  }
}


object SubjectUuidsSequenceAgent {
  def apply(systemContext: SystemComposite,
    ...
    val sequenceResourceName = ClassName(classOf[SubjectUuidsSequenceAgent])
