...
import util.locks.Locks
import util.com.udp.Udp
...
object DefaultSystemConfiguration {
  def apply(properties: Properties, dbName: String, arkName: String, host: String, port: Int) {
    ...
    org.agilewiki.core.DefaultSystemConfiguration(properties, dbName, arkName)

    Locks.defaultConfiguration(properties)
    Udp.defaultConfiguration(properties, host, port)
    ShortProtocol.defaultConfiguration(properties = properties,
