...
class ContextTypeQry(systemContext: SystemComposite, uuid: String)
  ...
  override protected def query: String = {
    ...
    if (!context.contains(name))
      ...
      val value = context.getSpecial(name)
      if (value.isInstanceOf[StringBuilder])
        typ = "variable"
      else if (value.isInstanceOf[String])
        typ = "constant"
    }
    context.setCon("type", typ)
    null
  }
}

object ContextTypeQry {
  val name = "qry-contextType"
  val cls = "org.agilewiki.command.cmds.ContextTypeQry"
}
