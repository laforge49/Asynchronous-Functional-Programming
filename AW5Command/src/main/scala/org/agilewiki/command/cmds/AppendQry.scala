...
class AppendQry(systemContext: SystemComposite, uuid: String)
  ...
  override protected def query: String = {
    ...
    if (!context.contains(name))
      return "Missing variable from context map: " + name
    if (!context.isVar(name))
      return "Not a variable: " + name
    val named = context.getVar(name)
    val sb = named.asInstanceOf[StringBuilder]
    if (!context.contains("value"))
      return "Missing argument from context map: value"
...
object AppendQry {
  val name = "qry-append"
  val cls = "org.agilewiki.command.cmds.AppendQry"
}
