...
abstract class SimpleQuery(systemContext: SystemComposite, uuid: String)
  ...
  private def invokeCmdMsg(msg: InvokeCmdMsg) {
    debug(msg)
    try {
      requester = msg.requester
      context= msg.context
      val err = query
      if (err != null) error(requester, err)
      else {
        val contextUpdateMsg = CommandCompletionMsg(msg.header)
