...
def minimize = context.get("comet.data.userEvent.minimize")

RolonUpdateRequest updateWindow = extendedContext.rolonUpdateRequest("webtop.window.minimize",context,window,"")
updateWindow.setAttribute("minimized",minimize)
extendedContext.updateRequests().add(updateWindow)
extendedContext.updateParameters().put("webtop.window.minimize.executed",true)