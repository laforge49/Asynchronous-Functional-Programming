...
desktopUuid = context.get("target.desktop.uuid")
RolonProxy desktop = extendedContext.rolonProxy(context, desktopUuid)

iconUuid = context.get("target.wrapper.uuid")
RolonProxy icon = extendedContext.rolonProxy(context, iconUuid)

newName = context.get("comet.data.userEvent.newName")

rename = extendedContext.renameRolonUpdateRequest("webtop.renameIcon",context,desktop,icon,newName,"")

extendedContext.updateRequests().add(rename)
extendedContext.updateParameters().put("webtop.renameIcon.executed",true)