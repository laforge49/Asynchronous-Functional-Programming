...
class SyncTest extends SpecificationWithJUnit {
  "nba" should {
    "run 100 * 100" in {
      val M = 100
      val N = 100
      ...
      val actors = new Array[SyncTestActor](1000)
      var j = 0
      while (j < M) {
        var actor: SyncTestActor = null
        var i = 0
        while (i < N) {
          actor = SyncTestActor(systemContext, actor)
          i = i + 1
        }
