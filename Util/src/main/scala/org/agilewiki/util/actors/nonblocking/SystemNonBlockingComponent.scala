...
class _NonBlocking(configurationProperties: Properties)
...
object NonBlocking {
  def defaultConfiguration(properties: Properties) {
    properties.put("nonBlockingDispatchThreads", "4")
  }
}

trait SystemNonBlockingComponent {
  this: SystemConfigurationComponent
    with SystemComposite =>

  private[nonblocking] lazy val dispatchManager = {
    val N = Configuration(this).requiredIntProperty("nonBlockingDispatchThreads")
    new _DispatchManager(N)
  }
}