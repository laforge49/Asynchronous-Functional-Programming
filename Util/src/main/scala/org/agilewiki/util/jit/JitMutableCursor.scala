...
case class JitMutableCursor(bytes: Array[Byte], var offset: Int) {
  def immutable = JitImmutableCursor(bytes, offset)

  def rewind {offset = 0}

  def skip(length: Int) {offset += length}

  def write(cursor: JitImmutableCursor, length: Int) {
    System.arraycopy(cursor.bytes, cursor.offset, bytes, offset, length)
    skip(length)
  }
