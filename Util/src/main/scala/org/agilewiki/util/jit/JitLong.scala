...
object JitLong {
  val defaultRoleName = JIT_LONG_ROLE_NAME

  def createJit(context: SystemComposite) = Jits(context).createJit(defaultRoleName).asInstanceOf[JitLong]
}

class JitLong extends Jit {
  ...
  def setLong(long: Long) {
    if ((isJitSerialized || dser) && getLong == long) return
    writeLock
    i = long
    dser = true
    jitUpdated(0, this)
