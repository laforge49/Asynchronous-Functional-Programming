...
class UpdateActor(systemContext: SystemComposite, uuid: String)
  ...
  override protected def firstStartElement(msg: StartElementMsg) {
    ...
    try {
      ...
      val cmdName = "upd-" + attributes.getValue("cmd")
      if (cmdName == null)
...
object UpdateActor extends TemplateActor("aw:update", classOf[UpdateActor].getName)
