...
class EncodedContentsActor(systemContext: SystemComposite, uuid: String)
        extends DropContentActor(systemContext, uuid) {
  var xmlComposer: XmlComposer = null
  var oldContentsEncoding = false
  var oldAttributesEncoding = false
  ...
  override protected def firstStartElement(msg: StartElementMsg) {
    ...
    try {
      ...
      oldContentsEncoding = xmlComposer.encodeContents
      oldAttributesEncoding = xmlComposer.encodeAttributes
      xmlComposer.encodeContents = false
      xmlComposer.encodeAttributes = false
  ...
  override protected def lastEndElement(msg: EndElementMsg) {
    debug(msg)
    xmlComposer.encodeContents = oldContentsEncoding
    xmlComposer.encodeAttributes = oldAttributesEncoding
    super.lastEndElement(msg)
  }
}

object EncodedContentsActor extends TemplateActor("aw:encodedContents", classOf[EncodedContentsActor].getName)
