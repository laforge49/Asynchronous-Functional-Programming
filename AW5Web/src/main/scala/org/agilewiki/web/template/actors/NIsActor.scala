...
class NIsActor(systemContext: SystemComposite, uuid: String)
        extends IsActor(systemContext, uuid) {
  override protected def test(attributes: Attributes) {
...
object NIsActor extends TemplateActor("aw:nis", classOf[NIsActor].getName)
