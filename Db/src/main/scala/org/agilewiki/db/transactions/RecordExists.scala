...
object RecordExists {
  def apply(db: Actor, batch: IncDes, recordKey: String, pathname: String) = {
    ...
    pn = recordKey + pn
    val jef = new RecordExistsFactory
    jef.configure(db)
...
class RecordExistsFactory extends IncDesStringFactory(DBT_RECORD_EXISTS) {
  override protected def instantiate = {
    val req = super.instantiate
    addComponent(new QueryRequestComponent(req))
    addComponent(new RecordExistsComponent(req))
    req
  }
}

class RecordExistsComponent(actor: Actor)
